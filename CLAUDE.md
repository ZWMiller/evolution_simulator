...
Every speciation event carries an `event_type` field. Valid values: `"cladogenesis_newborn"` (newborn fell outside all living species; promoted from candidate), `"cladogenesis_kmeans_subcluster"` (k-means split detector found reproductively-isolated sub-clusters), `"cladogenesis_bootstrap"` (founding species registration), `"anagenesis"` (in-place phenotype transformation). The phylogeny renders these four types distinctly.

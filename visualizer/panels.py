...
def _graph(figure, margin_bottom="4px") -> html.Div:
    return html.Div(
        dcc.Graph(figure=figure, config={"displayModeBar": "hover", "scrollZoom": True}, responsive=True),
...
def _config(state: dict, run: dict, day: int) -> list:
    ...
    items += [
        html.Div(
            "CONFIG",
            style=_s(
                color=BRIGHT, fontSize="12px", letterSpacing="0.15em", fontWeight="bold", marginBottom="10px"
            ),
        ),
        html.Pre(
...
def _global(state: dict, run: dict, day: int) -> list:
    ...
    spec_so_far = [ev for dn, evs in run["speciation_by_week"].items() for ev in evs if dn <= day]
    ...
    items += [
        html.Div(
            "GLOBAL STATS",
            style=_s(
                color=BRIGHT, fontSize="15px", letterSpacing="0.15em", fontWeight="bold", marginBottom="10px"
            ),
        ),
    ...
    if recent:
        items.append(
            _section(
                f"RECENT SPECIATIONS (last {len(recent)})",
                [
                    html.Div(
...
def _habitat(state: dict, run: dict, day: int) -> list:
    ...
    conns = [b if a == hab_id else a for a, b in run["connections"] if a == hab_id or b == hab_id]
    ...
    items += [
        html.Div(
            hab_name.upper(),
            style=_s(
                color=text_c, fontSize="15px", letterSpacing="0.12em", fontWeight="bold", marginBottom="2px"
            ),
        ),
        html.Div(
            hab_type, style=_s(color=DIMTEXT, fontSize="12px", letterSpacing="0.1em", marginBottom="10px")
        ),
        _section(
            "STATUS",
            [
...
def _species(state: dict, run: dict, day: int) -> list:
    ...
    if hab_scoped and hab_id:
        day_rows = [r for r in run["species_per_hab"].get(hab_id, {}).get(species, []) if r["week"] == day]
    else:
    ...
    items += [
        html.Div(
            species.upper(),
            style=_s(
                color=text_c, fontSize="15px", letterSpacing="0.1em", fontWeight="bold", marginBottom="2px"
            ),
        ),
        html.Div(
            f"in {hab_name}" if hab_scoped else "global",
    ...
    trait_rows = [
        _kv(t.replace("_", " "), f"{traits[t]:.4f}") for t in KEY_TRAITS if traits.get(t) is not None
    ]
    if trait_rows:
    ...
    items.append(
        _section(
            "GENERATION vs ADAPTATION",
            [
                _graph(figs.generation_adaptation(run, species, hab_id if hab_scoped else None)),
            ],
        )
    )
    ...
    if selected_trait:
        items.append(_graph(figs.species_trait(run, species, selected_trait, hab_id if hab_scoped else None)))

    # Birth events
    if births:
        items.append(
            _section(
                f"BORN TODAY ({len(births)})",
                [
                    html.Div(
...
def _edge(state: dict, run: dict, day: int) -> list:
    ...
    migs_fwd = [
        m
        for m in run["migrations_by_week"].get(day, [])
    ...
    items += [
        html.Div(
            "MIGRATION ROUTE",
            style=_s(
                color=BRIGHT, fontSize="15px", letterSpacing="0.12em", fontWeight="bold", marginBottom="4px"
            ),
        ),
        html.Div(
    ...
    for direction, mig_list, _label in [
